...
- `PATCH`: バグ修正、文言修正、定義の明確化などの小規模変更

リリース時には `VERSION` を更新し、対応する `vX.Y.Z` タグを付与する。

文書内の `aotp_version` は、導入対象プロジェクトの文書・Profile互換性を表す内部スキーマ版であり、リポジトリのリリース版とは別に扱う。

## Branch policy

AOTPの常設ブランチは次の2本を基本とする。

- `main`: 正式版・安定版。リリースされたAOTPの正本。
- `develop`: 開発版。次のリリースに向けた変更を集約する。

通常の変更は `develop` に反映し、リリース時に `develop` を `main` へマージする。`main` への直接変更は行わない。
