...
AOTPではTask、Proposal、Research、Design、Decision、Impl-noteなどの履歴MDを、後続エージェントが過去の判断と作業を復元するための外部記憶として扱う。しかし本文の構成や語彙が文書ごと・agentごとに大きく異なると、同じ情報を探すために毎回全文解釈が必要になる。

履歴MDについて、typeごとの書き方と共通部分の構造を統一する必要がある。

検討対象:

- 見出し順序の標準化
- 目的 / 前提 / 実施内容 / 結果 / 判断 / 未解決事項 / 検証証拠の区別
- 事実、推測、提案、決定済み事項を混同しない表記規則
- 実行前情報と実行後結果の分離
- 失敗・却下・未実施事項を消さずに記録する方法
- `unknown`、未確認、推測等の不確実性表現の統一
- source、commit、diff、test、関連文書への参照形式
- agent名、runtime、model、実行日時等の実行記録を本文とmetadataのどちらへ置くか
- 文書typeごとの最小テンプレート
- 長大化した履歴を分割する基準

例えばTaskでは、少なくとも次のような順序を候補とする。
...
Researchでは「調査目的 → 調査範囲 → 確認できた事実 → 推測 → 結論 → source」、Decisionでは「論点 → 選択肢 → 判断 → 理由 → 影響 → 再検討条件」のように、typeごとに安定した読み順を持たせることを検討する。

目的は文章表現そのものを画一化することではない。後続の人間・AIが文書を開いたとき、どこに何が書かれているかを予測でき、全文を読み直さなくても必要な履歴を復元できる状態を作ることである。
